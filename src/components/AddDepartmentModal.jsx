...
import { X, Building, Plus, FileText } from 'lucide-react';

export default function AddDepartmentModal({ onClose, onDepartmentAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onDepartmentAdded) {
        onDepartmentAdded(res.department);
      }
      onClose();
    } catch (err) {
      ...
      <div className="glass-panel w-full max-w-md rounded-2xl border border-slate-700/80 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              placeholder="e.g. Marketing, Quality Assurance"
              value={name}
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              rows={3}
              placeholder="Describe this department's primary organizational role and functions..."
              value={description}
